	...
	{		
		...
		case PREINSTALL_SESHOT:
		{
			if(rows == 0){
				...
				The_page_processing(rows,	 "    参数正在补拍");}
			else if(rows == 2){
				The_page_processing(rows, "按左键移到上一张");}
			else if(rows == 3){
				The_page_processing(rows, "按右键移到下一张");}
			else if(rows == 4){
				The_page_processing(rows,	 "按拍照键补拍");}
			else if(rows == 5){
				The_page_processing(rows,	 "                    ");}
			else if(rows == 6){
...
{
	Oled_EnlPrint(LCD_W-42, 5, "V1.0.1", "", ENGLISH);
}
