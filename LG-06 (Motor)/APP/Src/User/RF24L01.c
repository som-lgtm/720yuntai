...
void RF24L01_GPIO_Init(void)
{
	...
	*/
	...
	GPIO_InitStruct.Pin = LL_GPIO_PIN_5; // mosi
	GPIO_InitStruct.Mode = LL_GPIO_MODE_ALTERNATE;
	GPIO_InitStruct.Speed = LL_GPIO_SPEED_FREQ_LOW;
	GPIO_InitStruct.OutputType = LL_GPIO_OUTPUT_PUSHPULL;
	GPIO_InitStruct.Pull = LL_GPIO_PULL_UP;
	...
	GPIO_InitStruct.Pin = LL_GPIO_PIN_4; // miso
	GPIO_InitStruct.Mode = LL_GPIO_MODE_ALTERNATE;
...
/*	LL_SPI_SetMode(SPI1, LL_SPI_MODE_MASTER);
	...
	GPIO_OUT_init(GPIOA, LL_GPIO_PIN_11); // CSN
	GPIO_OUT_init(GPIOA, LL_GPIO_PIN_8); // CE
...
void RF24L01_GPIO_Init(void)
{
	GPIO_OUT_init(GPIOB, LL_GPIO_PIN_3); //SCK
	GPIO_OUT_init(GPIOB, LL_GPIO_PIN_5); // MOSI
	GPIO_INUP_Init(GPIOB, LL_GPIO_PIN_4); // MISO
	Irq_gpio_init(); //IRQ
	GPIO_OUT_init(GPIOA, LL_GPIO_PIN_15); // CSN
	GPIO_OUT_init(GPIOB, LL_GPIO_PIN_6); // CE
}
...
void Irq_gpio_init(void)
{
	...
	GPIO_INUP_Init(GPIOB, LL_GPIO_PIN_13); // DIAG
	
	EXTI_InitStruct.Line_0_31 = LL_GPIO_PIN_13;
...
void Irq_gpio_init(void)
{
	GPIO_INUP_Init(GPIOA, LL_GPIO_PIN_12); // IRQ
}
...
void NRF24L01_check( void )
{
	...
	while( 1 )
	{
		...
		else
		{
			...
			//Oled_EnlPrint(0, 1, "RF24L01 is not find", "", ENGLISH);
		}
...
//	(2)¥”≥Ã–Ú…œø¥£¨Œ™ ACK Õ®–≈ƒ£ Ω£¨“Ú¥À£¨Bit 0(0x1D.0) Œ™ 0
   ...
   // NRF24L01_Write_Reg( FEATRUE, 0x07 ); // 
    ...
    NRF24L01_Write_Reg( RF_CH, glob_para.wifi_id.wifi_rfc );             			//≥ı ºªØÕ®µ¿
//    NRF24L01_Write_Reg( RF_CH, 60 );             			//≥ı ºªØÕ®µ¿
    NRF24L01_Write_Reg( RF_SETUP, 0x27 );					//–ª∆µ ˝æ›¬ Œ™250K,¥À÷µ‘Ω∏ﬂ≤‡∑¢ÀÕæ‡¿Î‘ΩΩ¸∑Ò≤‡‘Ω‘∂
															// ∑¢ÀÕπ¶ƒ‹Œ™7dbm
    NRF24L01_Set_TxAddr( &glob_para.wifi_id.TX_adress[0], 5 );                      //…Ë÷√TX µÿ÷∑
    NRF24L01_Set_RxAddr( 0, &glob_para.wifi_id.TX_adress[0], 5 );                   //…Ë÷√P0π‹µ¿RX µÿ÷∑”√”⁄Ω” ’ACK”¶¥
...
void wifi24G_send_data(void)
{
	...
	{
		if(Usart11_shadow[index].shadow_send_size)break;
	}
	
	if(index >= LOCUS_BUFFER_MAX)return;
//	String_Printf(USART_2,&index,1);
	if(NRF24L01_Read_Reg( STATUS ) == RX_OK)return; // ’˝‘⁄Ω” ’ ˝æ› ±≤ªƒ‹∑¢ÀÕ
	...
	Send_status = NRF24L01_TxPacket( &Usart11_shadow[index].usart11_shadow_buffer[0], Usart11_shadow[index].shadow_send_size );
	if(Send_status == TX_OK)
	{
		...
		send_times = 0;
		send_overTime = 0;
		if(Alon_sendt)Alon_sendt -=1;
	}
	else if(Send_status == MAX_TX)//»Áπ˚ «÷ÿ∑¢¥Œ ˝≥¨¡ÀæÕ‘Ÿ∑¢
	{
		Usart11_shadow[0].shadow_send_size = 0;
	...
	else //»Áπ˚ «∑¢ÀÕ≥¨ ±÷ÿ–¬≥ı ºªØ∫Û÷ÿ∑¢ ˝æ›
	{
		if(send_overTime >= 5)
		{
			...
			{
				Usart11_shadow[i].shadow_send_size = 0;
				Alon_sendt = 0;
			}
		}
		send_overTime += 1;
...
//	if(usart_send.Tx_tag)return;
	sizes = NRF24L01_RxPacket(con_buffer);
	
	RF24L01_CE_ON;
	
	if(sizes)
	{
		if(checksum_verify(con_buffer, sizes-1) == FALSE)
		{
			con_buffer[1] = 0;
			return; 
		}
		String_Printf(USART_2,con_buffer,sizes);
		Get_data_from_controller(con_buffer);
		if(usart_send.send_interval < 10)usart_send.send_interval = 10; //∑¢ÀÕ∑Ω∑¢¿¥µƒ ˝æ› ’µΩ∫Û“™µ»∑¢ÀÕ∑Ω
															//«–ªªŒ™Ω” ’ƒ£ Ω∫Û≤≈ƒ‹ø™ º∑¢ ˝æ›∏¯∑¢ÀÕ∑Ω
		con_buffer[1] = 0;
	}
}
